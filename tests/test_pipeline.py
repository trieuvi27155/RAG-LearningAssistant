...
def test_doan_trich_noi_lai_dinh_nghia_bi_ranh_gioi_trang_cat_doi():
    """Định nghĩa bị ranh giới trang cắt đôi phải được nối lại: chunk neo ở cuối trang trước mở
    rộng hết mảng của trang rồi dừng thì phần còn thiếu không bao giờ được ghép."""
    cac_chunk = [
...
def test_mo_rong_khong_vuot_qua_so_trang_cho_phep(monkeypatch):
    """Chốt chặn cho slide: không có nó, một slide thưa chữ sẽ hút thêm vài slide xung quanh cho
    đầy ngân sách ký tự."""
...
def test_nguong_tuong_doi_giu_lai_ca_nhom_khi_diem_thap_deu(monkeypatch):
    """Cosine của E5 không phải thang đo tuyệt đối, nên ngưỡng phải là tương đối để không cắt
    sạch cả một nhóm đoạn vẫn liên quan ngang nhau."""
...
def test_trich_dan_ghi_du_khoang_trang_khi_doan_trich_vuot_ranh_gioi_trang():
    """Đoạn trích mở rộng sang trang liền kề thì trích dẫn phải ghi đủ khoảng trang, không chỉ
    ghi mỗi trang neo."""
    cac_chunk = [
...
def test_diem_rerank_qua_thap_thi_tu_choi(monkeypatch):
    """Điểm rerank quá thấp thì phải từ chối trả lời - rerank tách được câu lạc đề khỏi câu đúng
    chủ đề, còn cosine thì không."""
...
def test_diem_rerank_khong_sot_lai_giua_hai_luot(monkeypatch):
    ...
    pipeline.vector_store = VectorStore(dimension=SO_CHIEU)
...
def test_nhan_dien_dung_tieng_anh(cau_hoi):
    """Câu hỏi tiếng Anh phải được nhận đúng, không bị langdetect chấm nhầm sang một thứ tiếng
    Latin họ gần rồi bị trả lời bằng tiếng Việt."""
...
def test_nhan_dien_ngon_ngu_tat_dinh():
    """Nhận diện ngôn ngữ phải tất định - langdetect lấy mẫu ngẫu nhiên nên cùng một câu hỏi lúc
    được trả lời tiếng Anh lúc tiếng Việt."""
...
def test_tran_so_doan_anh(monkeypatch):
    """Cần trần số đoạn ảnh, vì mô tả ảnh khá dài sẽ lấn át các trang văn bản đúng và làm tụt
    Recall@K trên corpus thật."""
